...
- **Food Recognition Systems**: We provide state-of-the-art food recognition systems based on the Xception CNN architecture. These systems are designed to accurately classify food images into different granularity levels, from major categories to subcategories and final products.
...
The proposed food recognition system is based on the state-of-the-art **Xception** CNN architecture [8] due to the high performance obtained in computer vision tasks.  In this work, we follow the same training approach considered in [9]: using a pre-trained model with ImageNet [10], the last fully-connected layers are replaced by the corresponding number of classes (defined by each experiment). Then, all the weights are fixed up to the fully-connected layers and we re-train the model for 10 epochs. Finally, the whole network is trained again for 50 more epochs, choosing the best-performing model in terms of validation accuracy. We use the following features for all the experiments: Adam optimiser based on binary cross-entropy using a learning rate of 10<sup>-3</sup>, $\beta_1$ and $\beta_2$ of $0.9$ and $0.999$, respectively. In addition, training and testing are performed with an image size of 224x224. 

 **Three main experiments** are considered in the intra-database scenario, representing a different granularity defined by the number of categories, subcategories and products, respectively. From coarse-grained to fine-grained, we use in the first scenario all the **19 major categories**. In the second scenario, we use the **73 subcategories** derived from the categories. Finally, **840 final products** (from the initial 911 food products) are used in the third scenario as several of them represent the same food plate or do not contain more than 100 images among the 3 datasets. As a result, category-level performances show better results than any other categorisation level, obtaining **77.74% and 97.78%** for both Top-1 and Top-5 accuracy for the AI4Food-NutritionDB database, respectively. However, the food classification system works worse as the granularity becomes finer: for subcategory and product level, Top-1 Acc. decreases around 7% and 22%, respectively. For more information, please visit the cited article.

For reproducibility reasons, **we adopt the same experimental protocol considered in the collected databases**: we divide them into development and test subsets following each corresponding subdivision. In addition, the development subset is also divided into train and validation subsets. Nevertheless, **three of the collected databases do not contain this division**: FruitVeg81 and UECFood-256 databases do not provide an experimental protocol whereas Food-101 only contains development and test subsets. In this case, **we divide the databases into 3 non-overlapping subsets using a similar procedure as in [9]**: around 80\% of the images comprise the development subset (train and validation subsets are distributed around 80\% and 20\% of the development subset, respectively) and the rest of the images correspond to the test subset (around 20\%). It is important to remark that images are not repeated among the 3 subsets (train, validation, and test) in any of the 7 databases. Top-1 (Top-1 Acc.) and Top-5 classification accuracy (Top-5 Acc.) are used as evaluation metrics.

**Three different scenarios** are considered for the intra-database evaluation of the AI4Food-NutritionDB database. **Each of the scenarios represents a different granularity defined by the number of categories (19), subcategories (73), and final products (893)**, respectively. Category level performances show better results than any other categorisation level, obtaining **77.74\% and 97.78\% for both Top-1 and Top-5 Acc.** for the AI4FoodDB\_Nutrition database. However, the food classification system works worse as the granularity becomes finer: for subcategory and product level, Top-1 Acc. decreases around 7\% and 22\%, respectively. This is mainly due to the similar appearance existed among different subcategories (e.g., 'White Meat' and 'Red Meat'),  final products (e.g., 'Pizza Carbonara' and 'Pizza Pugliese'), or even the same food cooked in several manners (e.g., 'Baked Salmon' and 'Cured Salmon'). Regarding each specific database, the FruitVeg-81 database shows the best results, classifying almost perfectly the different fruits and vegetables (overcoming 95\% Top-1 and Top-5 Acc. for all categorisation levels). Contrarily, the VIPER-FoodNet database obtains the worst results at each categorisation level as images sometimes contain food products with different ingredients (e.g., different types of beans, meat, or pasta). 
...
**[9]** R. Tolosana, S. Romero-Tapiador, R. Vera-Rodriguez, E. Gonzalez-Sosa, and J. Fierrez, DeepFakes Detection Across Generations: Analysis of Facial Regions, Fusion, and Performance Evaluation. *Engineering Applications of Artificial Intelligence*, vol. 110, p. 104673, 2022.
...
After downloading all 7 databases, place them in the same path as this GitHub repository (previously cloned or downloaded). Then, download the **[AI4Food-NutritionDB.txt](http://atvs.ii.uam.es/atvs/2022/AI4Food-NutritionDB/)** and also place it in the same path as before. Finally, run **database_generation.py** and wait until the database is properly generated! Once this last step is done, the AI4Food-NutritionDB database will be available. As mentioned in the paper, the database is organised into 19 major categories, 73 subcategories, and 911 final products.
...
**NOTE**: **test images are located in the media/sample folder**. Place food images inside this folder or change the path of the **test_dir** parameter (in [food_recognition_system.py](https://github.com/BiDAlab/AI4Food-NutritionDB/blob/4497e6756b88e09faa91592577a86966a96be697/src/food_recognition_system.py#L89) file).
